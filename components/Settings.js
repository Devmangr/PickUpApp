...
import { View, Text, TextInput, StyleSheet, Pressable, Alert, Image, Modal } from 'react-native';
...
const SettingsScreen = () => {
  const { wsHost, wsPort, wsRoot, wsUser, wsPass, seriesM, seriesT, priceList, branch, branchDescr, 
          updateWsHost, updateWsPort, updateWsRoot, updateWsUser, updateWsPass, updateSeriesM, updateSeriesT, updatePriceList, updateBranch } = useAppContext();
  ...
  const fetchBranches = async () => {
    ...
    try {
      const body = JSON.stringify({"sql":"select codeid, description from branch order by codeid","dbfqr":true,"params":[]});
  ...
  const checkConn = async () => {
    ...
    try {
      ...
      Alert.alert('Σφάλμα',`Τα στοιχεία σύνδεσης δεν είναι σωστά. Error: ${error}. `, [
        {
          text: 'Ok',
  ...
  const handleSaveSettings = () => {
    ...
    try {
      ...
      Alert.alert('Σφάλμα',`Σφάλμα στην αποθήκευση ρυθμίσεων. Μήνυμα: ${error}. `, [
        {
          text: 'Ok',
    ...
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <Pressable style={styles.press} onPress={() => setIsPopupVisible(true)}> 
          <Image style={styles.img} source={require('../assets/—Pngtree—vector refresh icon_4187318.png')}/>
        </Pressable>
      </View>
      <View style={styles.row}>
      ...
      <Modal 
        ...
        <View style={styles.popupContainer}>
          ...
          <Text style={{textAlign:'center', fontSize:16}}>Επέλεξε κατάστημα.</Text>
            <View style={styles.popup}>
            <Text style={{marginBottom:20, fontSize:16}}>{branch}. {branchDescr}</Text>
              <Picker
                style={styles.picker}
                ...
                {Array.isArray(branches) && branches.length > 0 ? (
                  ...
                  <Picker.Item label='Μη διαθέσιμα υποκαταστήματα' value={null} />
                )}
              </Picker>
            </View>
            <Pressable style={styles.btn} onPress={()=>setIsPopupVisible(false)}>
